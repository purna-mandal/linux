...
static int pic32_regulator_probe(struct platform_device *pdev)
{
	...
	np = pdev->dev.of_node;
	of_property_read_u32(np, "microchip,regulator-vin-select",
			     &vin_supply_src);
	...
	of_property_read_u32(np, "microchip,regulator-vin-uV", &uvin_supply);
	if (!uvin_supply) {
		dev_err(&pdev->dev, "incorrect input supply\n");
		return -EINVAL;
	}

	of_property_read_u32(np, "microchip,regulator-vout-select", &vout_range);
...
static const struct of_device_id pic32_of_match[] = {
	{ .compatible = "microchip,pic32mzda-regulator",},
	{ },
};
