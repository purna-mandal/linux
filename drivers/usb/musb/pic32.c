...
struct pic32_musb {
	...
	struct timer_list	timer;      /* otg workaround timer */
};
...
static int pic32_musb_set_mode(struct musb *musb, u8 mode)
{
	...
	glue = dev_get_drvdata(dev->parent);
	cr = musb_readl(glue->cru, USBCRCON);
	switch (mode) {
	case MUSB_HOST:
		cr &= ~USBCRCON_USBIDVAL;
		cr |= USBCRCON_USBIDOVEN;
		musb_writel(glue->cru, USBCRCON, cr);
...
static int pic32_add_musb_device(struct pic32_musb *glue,
				 struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "using fifo mode %d\n", fifo_mode);
	switch (fifo_mode) {
	case 1:
	...
	/* Device-tree provides in mA, but MUSB wants it as per USB spec */
	of_property_read_u32(np, "microchip,power", (u32 *)&pdata.power);
	pdata.power /= 2;
...
static int pic32_musb_probe(struct platform_device *pdev)
{
	...
	dev_oc = of_parse_phandle(pdev->dev.of_node, "usb-overcurrent", 0);
	if (!dev_oc) {
		dev_err(&pdev->dev, "'usb-overcurrent' not found\n");
		return -EINVAL;
	}

	glue->oc_irq = irq_of_parse_and_map(dev_oc, 0);
	if (!glue->oc_irq) {
		dev_err(&pdev->dev, "cannot get overcurrent irq!\n");
		ret = -EINVAL;
		goto err_put_oc;
	}
	...
	ret = pic32_add_musb_device(glue, pdev);
	if (ret) {
