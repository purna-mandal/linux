...
/*
 * Register offsets are straight out of the datasheet so the pic32.h offsets are
 * not used for the PPS base.
 */
#define PPS_BASE 0xbf800000
...
{
	void __iomem *pps_base = (void __iomem *)(unsigned long)(PPS_BASE);
	int i;
	...
	puts("Input PPS function invalid\n");
}
...
void pic32_pps_output(int function, int pin)
{
	...
	for (i = 0; i < ARRAY_SIZE(output_pin_reg); i++) {
		if (output_pin_reg[i].pin == pin) {
			__raw_writel(function, pps_base + output_pin_reg[i].reg);
			return;
		}
	}

	puts("Output PPS pin invalid\n");
}
