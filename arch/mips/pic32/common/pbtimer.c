					      ...
					      struct clk **parent_clk_p)
{
	...
	struct clk_hw *parent_clk, *best_clk = NULL;

	for (c = 0; (c < clk_hw_get_num_parents(&timer->hw)) && delta; c++) {
		parent_clk = clk_hw_get_parent_by_index(&timer->hw, c);
...
static const struct of_device_id pic32_timer_match[] = {
	{ .compatible = "microchip,pic32mzda-timer", .data = &pic32_data[0], },
	{ .compatible = "microchip,pic32mzda-timerA", .data = &pic32_data[1], },
	{ .compatible = "microchip,pic32mzda-timer-adc", .data = &pic32_data[2],},
	{ .compatible = "microchip,pic32mzda-timer32", .data = &pic32_data[3],},
	{},
};
#endif
